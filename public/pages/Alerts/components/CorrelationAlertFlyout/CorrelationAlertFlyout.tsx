...
export class CorrelationAlertFlyout extends React.Component<
  CorrelationAlertFlyoutProps,
  CorrelationAlertFlyoutState
> {
  ...
  getFindings = async () => {
    ...
    try {
      ...
      const relatedFindings = await DataStore.findings.getFindingsByIds(findingIds);
    ...
    return [
      ...
      },
      {
        ...
        render: (queries: Query[], item: any) => {
          ...
          return <EuiBadge key={key}>{tag ? formatRuleType(tag) : ''}</EuiBadge>;
        },
      },
    ];
  }
