...
export const CreateDecoders: React.FC<FormDecodersProps> = (props) => {
  ...
  const idDecoder = props.match.params;
  ...
  useEffect(() => {
    const fetchDecoder = async () => {
      if (idDecoder && idDecoder.id) {
        setIsLoading(true);
        try {
          const response = await DataStore.decoders.getDecoder(idDecoder.id);
          ...
          errorNotificationToast(
            ...
            "retrieve",
            "decoder",
            `There was an error retrieving the decoder with id ${idDecoder.id}.`,
          );
        } finally {
          setIsLoading(false);
        }
