...
| [#497](https://github.com/wazuh/wazuh-dashboard-security-analytics/issues/497) | Fixed the side navigation not updating the displayed page after navigating away via the Ruleset management breadcrumb |
...
| [#502](https://github.com/wazuh/wazuh-dashboard-security-analytics/issues/502) | Fixed the search box not matching partial text on KVDBs and on decoder titles: rules, decoders and KVDBs now match the typed text anywhere inside the value, case-insensitively, through a single shared query builder, the invalid search warning tells the user what to type instead when the rejected field is unsupported and leaves a grammar error to its own explanation, and a hand-typed filter such as `level:high` applies instead of being silently dropped in favour of the parenthesized `level:(high)` form the filter selectors write. The rule level filter clause is now `level`, matching the document field and the column, instead of `severity`. The KVDBs list no longer sends one search request per keystroke, ignoring its own debounce, rules, decoders and KVDBs each load with a single search request instead of a duplicate one, a typed filter value debounces like free text instead of firing a request per keystroke, a filter applied while the free text changes is no longer dropped from the URL, a second integration navigation on the same page applies instead of being ignored, typing and then leaving the page no longer pulls the user back to the list, and a typed `*` matches integration names the same way it matches titles |
