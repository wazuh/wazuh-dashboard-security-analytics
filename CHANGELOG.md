...
| [#482](https://github.com/wazuh/wazuh-dashboard-security-analytics/issues/482) | Added a Visual Editor to the decoder form as an alternative to the YAML editor, covering the whole decoder document (`name`, `metadata`, `parents`, `definitions`, `check`, `parse|<field>` and the `normalize` entries) and validated against the engine JSON Schema; anything the form cannot represent is kept exactly as it is and stays editable as YAML in place |
