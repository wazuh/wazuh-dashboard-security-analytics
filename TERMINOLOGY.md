...
| `Edit` | — | The row action that opens an item for editing. Description: `Edit <entity>`. |
...
| `Rule status` | — | A rule's maturity, from the rule's `status` field (`experimental`, …). Distinct from `Status`. |
| `Status` | — | Whether an entity is enabled or disabled. |
...
| `Space` | — | The active scope only: Draft, Test, Custom, Standard. |
...
state are both visible in the rule flyout — while `Space`, `Integration`, `Title`,
