...
> **repo-specific (wazuh-dashboard-security-analytics):** this is a **single OSD
...
and recommended manual steps). `tmp/` is already git-ignored in this repo.
...
- CVE issues usually live in **`internal-devel-requests`** → `## Description`
  has no closing reference and **no CHANGELOG entry**. If the CVE issue is
