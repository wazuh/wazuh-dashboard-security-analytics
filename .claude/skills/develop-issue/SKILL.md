...
- **Internal** — URL contains `internal-devel-request` (e.g.
  ...
  (`## Description` has no closing reference) and there is **no CHANGELOG
  entry**.
- **Public** — any other repo (e.g. `wazuh/wazuh-dashboard-security-analytics`):
...
`common` (shared) — plus `models`/`types`/`config.ts` when relevant. Ask the user
only if genuinely blocked; otherwise proceed with reasonable defaults.
...
- This is a fork of `opensearch-project/security-analytics-dashboards-plugin`:
  match the **upstream filename convention** (PascalCase for components, e.g.
  `ContentPanel.tsx`), TypeScript, English everywhere. On upstream syncs, Wazuh
  content wins.

### 3. Tests (colocated)

Add or update unit tests as `*.test.ts` / `*.test.tsx` **next to** the changed
source files (many use jest snapshots under `__snapshots__/`). New functionality
must include testing.

> **repo-specific (wazuh-dashboard-security-analytics):** this plugin's scripts
> run from **inside the `wazuh-dashboard` checkout** at `plugins/<this-plugin>`
> (they reference `../../node_modules/.bin/jest`). Bootstrap the platform once
> (`yarn osd bootstrap` from the `wazuh-dashboard` root), then run
> `yarn test:jest` here; refresh snapshots with `yarn test:jest:update-snapshots`.
...
- The filled [`.github/PULL_REQUEST_TEMPLATE.md`](../../../.github/PULL_REQUEST_TEMPLATE.md)
  body, with `## Description` explaining the change (`Closes #<n>` / issue URL
  for public issues, **no closing reference** for internal-devel-requests
  issues), `### Tests Introduced` and `### How to Test` describing the added
  tests, and a screenshot/video reminder under `### Results and Evidence` for
  any UI change.
